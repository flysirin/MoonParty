...
router_set_win_lives = Router()
router_set_win_lives.callback_query.filter(StateFilter(FSMHost.set_win_lives))


@router_set_win_lives.callback_query(Text("_plus_lives_pressed_"))
async def plus_n_sec(callback: CallbackQuery, state: FSMContext, bot: Bot):
    host_data = await state.get_data()
    host_data['settings']['win_lives'] += 5
    start_lives = host_data['settings']['win_lives']
    await state.update_data(data=host_data)
    await callback.message.edit_text(text=f"{HOST_LEXICON['Win lives:']} {start_lives}",
                                     reply_markup=host_keyboards.set_lives_inline_kb())


@router_set_win_lives.callback_query(Text("_minus_lives_pressed_"))
async def minus_n_sec(callback: CallbackQuery, state: FSMContext, bot: Bot):
    host_data = await state.get_data()
    if host_data['settings']['win_lives'] <= 10:
        return await callback.answer(text=HOST_LEXICON["Exception win lives"], show_alert=True)
...
@router_set_win_lives.callback_query(Text("_exit_settings_menu_pressed_"))
